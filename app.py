...
def route_intent(intent: str, entities: dict, raw_text: str) -> str:
    ...
    try:
        if intent == "weather":
            ...
            return "Sorry, couldn't play music."
...
with st.sidebar:
    ...
    nav_options = ["Home", "Chat", "Weather", "Music", "Email", "Chess", "Memory", "Personality", "Settings"]
...
def page_music():
    st.title("Music")
    st.caption("Search and play your favorite music")
    
    st.divider()
    
    # Music controls
    query = st.text_input("Search Song or Artist", placeholder="Enter song name or artist...")
        ...
        if st.button("Play", use_container_width=True, type="primary"):
            if query:
                with st.spinner(f"Searching for {query}..."):
                    if music.play(query):
                        st.success(f"Now playing: {query}")
                        st.session_state.now_playing = query
                    else:
                        st.error("Could not find or play the requested music")
            else:
                st.warning("Enter a song or artist first")
    ...
    with col3:
        if st.button("Pause/Resume", use_container_width=True):
            music.pause() if hasattr(music, 'pause') else st.info("Pause not available")

    st.divider()
    
    # Now playing status
    if music.is_playing():
        playing = st.session_state.get("now_playing", "Unknown")
        with st.container(border=True):
    ...
    with rec_col1:
        if st.button("Relaxing Music", use_container_width=True):
            music.play("relaxing music")
            st.session_state.now_playing = "Relaxing Music"
            st.rerun()
    
    with rec_col2:
        if st.button("Focus Music", use_container_width=True):
            music.play("focus music")
            st.session_state.now_playing = "Focus Music"
            st.rerun()
    
    with rec_col3:
        if st.button("Upbeat Music", use_container_width=True):
            music.play("upbeat music")
            st.session_state.now_playing = "Upbeat Music"
            st.rerun()
